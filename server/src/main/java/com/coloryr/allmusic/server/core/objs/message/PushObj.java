...
public class PushObj {
    ...
    public void init() {
        ...
        if (agree == null)
            agree = "<light_purple>[AllMusic]<yellow>你同意插歌";
        if (bqAgree == null)
            bqAgree = "<light_purple>[AllMusic]<yellow>" + ARG.player + "同意插歌，共有" + ARG.count + "名玩家同意插歌，需要至少" + ARG.countAll + "名玩家同意才会插歌。";
        if (arAgree == null)
            arAgree = "<light_purple>[AllMusic]<yellow>你已申请插歌";
        if (timeOut == null)
            timeOut = "<light_purple>[AllMusic]<yellow>插歌时间结束";
        if (doPush == null)
        ...
        if (idErr == null)
            idErr = "<light_purple>[AllMusic]<red>错误的歌曲序号";
        if (pushErr == null)
            pushErr = "<light_purple>[AllMusic]<red>这首歌已经是下一首播放了";
        if (err1 == null)
            err1 = "<light_purple>[AllMusic]<red>你没有发起插歌";
        if (err2 == null)
            err2 = "<light_purple>[AllMusic]<red>你发起的插歌已超时";
        if (err3 == null)
            err3 = "<light_purple>[AllMusic]<red>发起失败，已经有人发起了插歌投票";
        if (cancel == null)
