...
public class CommandDelete extends ACommand {
    ...
    public void execute(Object sender, String name, String[] args) {
        ...
        if (!args[1].isEmpty() && Function.isInteger(args[1])) {
            ...
            if (music == 0) {
                AllMusic.side.sendMessage(sender, "<light_purple>[AllMusic3]<dark_green>请输入有效的序列ID");
                return;
            }
            if (music > PlayMusic.getListSize()) {
                AllMusic.side.sendMessage(sender, "<light_purple>[AllMusic3]<dark_green>序列号过大");
                return;
            }
            PlayMusic.remove(music - 1);
            AllMusic.side.sendMessage(sender, "<light_purple>[AllMusic3]<dark_green>已删除序列" + music);
        } else {
            AllMusic.side.sendMessage(sender, "<light_purple>[AllMusic3]<dark_green>请输入有效的序列ID");
        }
    }
