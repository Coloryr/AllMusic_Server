...
public final class Header {
    ...
    void read_header(Bitstream stream, Crc16[] crcp) throws Exception {
        ...
        do {
            ...
            if (h_layer == 1) h_number_of_subbands = 32;
                ...
                if (h_mode != SINGLE_CHANNEL) if (channel_bitrate == 4) channel_bitrate = 1;
    ...
    public String sample_frequency_string() {
        switch (h_sample_frequency) {
            case THIRTYTWO:
                if (h_version == MPEG1) return "32 kHz";
                else if (h_version == MPEG2_LSF) return "16 kHz";
                else // SZD
                    return "8 kHz";
            case FOURTYFOUR_POINT_ONE:
                if (h_version == MPEG1) return "44.1 kHz";
                else if (h_version == MPEG2_LSF) return "22.05 kHz";
                else // SZD
                    return "11.025 kHz";
            case FOURTYEIGHT:
                if (h_version == MPEG1) return "48 kHz";
                else if (h_version == MPEG2_LSF) return "24 kHz";
                else // SZD
                    return "12 kHz";
        }
        return (null);
